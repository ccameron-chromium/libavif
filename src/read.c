...
static avifBool avifParseToneMappedImageBox(avifGainMapMetadata * metadata, const uint8_t * raw, size_t rawLen, avifDiagnostics * diag)
{
    ...
    AVIF_CHECK(avifROStreamReadU16(&s, &minimum_version));
    if (version != 0) {
        avifDiagnosticsPrintf(diag, "Box[tmap] has unsupported minimum_version [%u]", minimum_version);
        return AVIF_FALSE;
    }
    uint16_t writer_version;
    AVIF_CHECK(avifROStreamReadU16(&s, &writer_version));
